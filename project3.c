...
static void *createNode(void *args) {
  ...
  if(nodes_left==0)
    pthread_cond_broadcast(&nodes);
  else
    pthread_cond_wait(&nodes,&lock);
          ...
          for(int j=-5;j<5;j++)
            if(xy->x+a>-1&&xy->y+j>-1&&xy->x+a<100&&xy->y+j<100)
              if(grid[xy->x+a][xy->y+j].channel==grid[xy->x][xy->y].channel)
                {
                //printf("Blocked");
                pthread_mutex_lock(&grid[xy->x+a][xy->y+j].lock);
                  if(!(a==0&&j==0))
                  {
        ...
        for(int a=-5;a<5;a++)
          for(int j=-5;j<5;j++)
            if(xy->x+a>-1&&xy->y+j>-1&&xy->x+a<100&&xy->y+j<100)
              if(grid[xy->x+a][xy->y+j].channel==grid[xy->x][xy->y].channel)
                {
                pthread_mutex_unlock(&grid[xy->x+a][xy->y+j].lock);
                  //printf("UNLOCK\n");
                }
          flag=0;
        //pthread_cond_signal(&finish);
          pthread_mutex_unlock(&grid[xy->x][xy->y].lock);  
      }
    }
